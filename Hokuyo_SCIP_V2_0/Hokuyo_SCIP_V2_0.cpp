...
Hokuyo_SCIP_V2_0::Hokuyo_SCIP_V2_0(void)
{
	hOprosAPI = NULL;
	uart = NULL;
}
...
int32_t Hokuyo_SCIP_V2_0::Initialize(OPRoS::Property parameter)
{
	...
	if(SetProperty(parameter) != API_SUCCESS) {
		return API_ERROR;
	}
...
int32_t Hokuyo_SCIP_V2_0::Finalize(void)
{
	FinalizeUART();
...
int32_t Hokuyo_SCIP_V2_0::Enable(void)
{
...
int32_t Hokuyo_SCIP_V2_0::Disable(void)
{
...
int32_t Hokuyo_SCIP_V2_0::SetProperty(OPRoS::Property parameter)
{
	...
	if(parameter.FindName("MaximumStep") == false)		return API_ERROR;
	if(parameter.FindName("StartStep") == false)		return API_ERROR;
	if(parameter.FindName("EndStep") == false)			return API_ERROR;
	if(parameter.FindName("DeltaStep") == false)		return API_ERROR;
...
//		laserScannerData.push_back((double)scannedData[i] * 0.001);
//	}

	//170801 mskim
	OPRoS::Float64Array sensorData;
	sensorData.status=API_SUCCESS;

	for(int i = 0; i < scanStepSize; i++) {
		sensorData.data.push_back(scannedData[i] * 0.001);
	}
	laserScannerData.push_back(sensorData);	//남욱이 수정 17/09/09
	//laserScannerData.push_back(sensorData);
//std::cout<<"HOKUYO GETSENSORVALUE TEST04: scanStepSize="<<scanStepSize<<std::endl;
...
//std::cout<<"HOKUYO INITIALIZEURT TEST04"<<std::endl;

	return API_SUCCESS;
}

int Hokuyo_SCIP_V2_0::FinalizeUART(void)
{
	if(uart != NULL) {
		uart->Disable();
		uart->Finalize();
...
int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_SendPack(unsigned char *data, int size)
{
	if(uart == NULL) {
		return -1;
	}
...
int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_RecvPack(unsigned char *data, int size)
{
//std::cout<<"HOKUYO RECVPACK TEST01: size="<<size<<std::endl;
	if(uart == NULL) {
		return -1;
	}
	...
	for(int i = 0; i < size; i += ret) {
		...
		if(ret < 0) {
			...
			if(*(data + count++) == ASCII_LF) {
				...
				if(isLast == true) {
//std::cout<<"HOKUYO RECVPACK TEST02-2-1-1: count="<<count<<std::endl;
					return count;
				}
				else
					isLast = true;
...
int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_CheckSum(unsigned char *data, int size)
{
...
int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_ChangeVer2_0(void)
{
#ifndef DUMMY
...
#else
	printf ("Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_ChangeVer2_0()\n");
#endif
	return 0;
}

int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_Reset(void)
{
#ifndef DUMMY
...
#else
	printf ("Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_Reset()\n");
#endif
	return 0;
}
...
#else
	printf ("Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_SetBaudRate(%d)\n", baudRate);
#endif
	return 0;
}

int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_Enable(unsigned char *status)
{
#ifndef DUMMY
...
#else
	status[0] = 0;
	printf ("Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_Enable(%d)\n", status[0]);
#endif
	return 0;
}

int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_Disable(void)
{
#ifndef DUMMY
...
#else
	printf ("Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_Disable()\n");
#endif
	return 0;
}
...
int Hokuyo_SCIP_V2_0::Hokuyo_SCIPV2_0_GetLastData(void)
{

#ifndef DUMMY
	unsigned char sendData[32] = {0, };
	unsigned char recvData[4096] = {0, };  //ORIGINAL
	...
	//STATUS SHOULD BE '00' CHECK STATUS INFORMATION '01' '02' '03' '04' '05' '10' ETC.
	if(!((recvData[nSend] == '0') && (recvData[nSend + 1] == '0'))) { //ORIGINAL
		return -1;  
	}
