...
ReturnType LaserScannerComp::SetProperty(OPRoS::Property parameter)
{
	if(laserScanner == NULL) {
		PrintMessage("ERROR : LaserScannerComp::SetProperty() -> The handle of API is not created.\n");
		return lastError = OPROS_PRECONDITION_NOT_MET;
	}

	if(laserScanner->SetProperty(parameter) < 0) {
...
OPRoS::Property LaserScannerComp::GetProperty()
{
	OPRoS::Property parameter;

	if(laserScanner == NULL) {
		PrintMessage("ERROR : LaserScannerComp::GetProperty() -> The handle of API is not created.\n");
		lastError = OPROS_PRECONDITION_NOT_MET;
	}

	if(laserScanner->GetProperty(parameter) < 0) {
...
std::vector<OPRoS::Float64Array> LaserScannerComp::GetSensorValue()
{
...
{
	std::cout<<"onInitial Test"<<std::endl;
	OPRoS::Property parameter;
	...
	//	API Casting
	laserScanner = dynamic_cast<LaserRangeFinder *>(getOprosAPI());
	if(laserScanner == NULL) {
